...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = (activated_output - y) * np.vectorize(
            output_layer_activation_derivative)(activated_output)
        hidden_layer_error = np.multiply(np.dot(self.hidden_to_output_weights.transpose(), output_layer_error),
                                         np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input))

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.dot(
            hidden_layer_activation, output_layer_error.T)
        input_to_hidden_weight_gradients = np.dot(
        ...
        self.hidden_to_output_weights = self.hidden_to_output_weights - \
            self.learning_rate * hidden_to_output_weight_gradients.T
