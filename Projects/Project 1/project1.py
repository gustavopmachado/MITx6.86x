...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    if label*(np.sum(np.multiply(np.transpose(feature_vector), theta)) + theta_0) < 1:
        return 1 - label*(np.sum(np.multiply(np.transpose(feature_vector), theta)) + theta_0)
    return 0
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    hinge = np.empty([feature_matrix.shape[0]])
    for i, feature_vector in enumerate(feature_matrix):
        hinge[i] = hinge_loss_single(feature_vector, labels[i], theta, theta_0)
...
def pegasos_single_step_update(
    ...
    if label*(np.dot(np.transpose(current_theta), feature_vector) + current_theta_0) <= 1:
