...
def SNN():
    ...
    Z = np.array([W[0]*x, W[1]*np.maximum(0, W[0]*x) + b])
    y = sigmoid(Z[1])

    t = 1

    return 0.5*(y - t)**2, (y-t)*np.exp(-Z[1])/((1 + np.exp(-Z[1]))**2)*W[1]*x, (y-t)*np.exp(-Z[1])/((1 + np.exp(-Z[1]))**2)*Z[0], (y-t)*np.exp(-Z[1])/((1 + np.exp(-Z[1]))**2)
